...
internal fun createCustomRadioEngine(
    ...
    return when (planEntry.backend) {
        Settings.CustomRadioBackend.LIBVLC -> LibVlcCustomRadioEngine(
            context = context,
...
private class ExoCustomRadioEngine(
    context: Context,
...
private class LibVlcCustomRadioEngine(
    ...
    private var started = false
    // Desired audio delay in microseconds. Persisted here so we can re-apply it whenever
    // LibVLC fires Playing (including silent ICY reconnects that reset audioDelay to 0).
    private var targetAudioDelayUs: Long = initialAudioDelayMs * 1_000L

    private fun applyTargetAudioDelay() {
        if (!released && mediaPlayer.audioDelay != targetAudioDelayUs) {
    ...
    override fun setAudioDelay(delayMs: Long) {
        if (!released) {
            targetAudioDelayUs = delayMs * 1_000L  // microseconds
            applyTargetAudioDelay()
        }
    }
